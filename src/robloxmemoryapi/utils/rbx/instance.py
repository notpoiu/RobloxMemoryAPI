...
class Head:
    ...
    @property
    def MeshData(self):
        ptr = self.memory_module.get_pointer(self.ToMeshData, self.offset_base["MeshData"])
    ...
    def GetMeshData(self, id: str | tuple = None, max_nodes: int = 100000) -> dict[str, dict[str]]:
        ...
        while self.Node and visited < max_nodes:
            ...
            if mesh and flag:
                ...
                if vertex_start and vertex_end and face_start and face_end and vertex_end > vertex_start and face_end > face_start:
...
class MeshContentProviderService(ServiceBase):
    ...
    def _ptr(self, address, offset_name=0):
        if self.failed:
            return 0
        return self.memory_module.get_pointer(address, self.offset_base[offset_name] if isinstance(offset_name, str) else offset_name)

    @property
    def Cache(self):
        return self._ptr(self.instance.raw_address, 0xF0) #Cache offset

    @property
    def LRUCache(self):
        return self._ptr(self.Cache, "LRUCache")
    ...
    @property
    def Head(self):
        ptr = self._ptr(self.LRUCache, 0x08)
